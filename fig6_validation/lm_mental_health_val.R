...
results <- data.frame()
# 1. 数据准备 ----------------------------------------------------------------
df <- read_csv("pathway/validation/mental_health_val.csv")

all_cols <- colnames(df)

start_physio <- which(all_cols == "Mental_distress")
end_physio <- which(all_cols == "Well_being")
...
# 保存结果
write.csv(results, "pathway/validation/lm_ref_health_mental_health_val.csv", row.names = FALSE)
